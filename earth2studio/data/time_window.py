...
class TimeWindow:
    ...
    **Datasource contract**
    -----------------------
    The wrapped ``datasource`` must implement the :class:`DataSource` protocol:

    - Accepts ``(time, variable)`` where:

      - ``time`` is a single ``datetime``, a list of ``datetime`` objects,
        or a ``TimeArray``.
      - ``variable`` is a string, list of strings, or ``VariableArray``.

    - Returns an :class:`xarray.DataArray` with at least the following:
    ...
    **Note**: This wrapper only supports :class:`DataSource` (single time
    dimension). It does **not** support ``ForecastSource`` (which has both
    init time and lead time).

    **Validation**: The wrapper is transparent and does not add validation beyond what
    the underlying datasource provides. Different datasources handle missing data
    differently (e.g., GFS logs warnings and returns NaN, while CMIP6 raises ValueError).
    The wrapper preserves this behavior.
    ...
    datasource : DataSource
        The underlying datasource to wrap. Must implement the DataSource protocol
        (single time dimension). ForecastSource is not supported.
    ...
    def _reorder_if_needed(
        self, result: xr.DataArray, variable_list: list[str]
    ) -> xr.DataArray:
        """Reorder variables based on group_by parameter.

        Parameters
        ----------
        result : xr.DataArray
            Data array with variables in natural concatenation order
        variable_list : list[str]
            List of requested variable names (without suffixes)

        Returns
        -------
        xr.DataArray
            Reordered data array if group_by='variable', otherwise unchanged
        """
        if self.group_by == "variable":
            # Reorder: group by variable, then by offset
            # From: [var1_offset1, var2_offset1, var1_offset2, var2_offset2, ...]
            # To:   [var1_offset1, var1_offset2, ..., var2_offset1, var2_offset2, ...]
            reordered_vars = self._expand_variables_with_suffixes(variable_list)
            result = result.sel(variable=reordered_vars)
        # else: group_by == "offset", keep natural order from concatenation
