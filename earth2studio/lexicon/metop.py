...
class MetOpIASILexicon(metaclass=LexiconType):
    ...
    # on S3 (e.g. diag_iasi_metop-c_ges).  These are **0-based** channel
    # indices (0..8460) matching the ``channel_index`` output column of both
    # :class:`~earth2studio.data.MetOpIASI` and
    # :class:`~earth2studio.data.UFSObsSat`.  They span all three IASI bands.
    # fmt: off
...
class MetOpMTGLexicon(metaclass=LexiconType):
    ...
    def get_item(cls, val: str) -> tuple[str, Callable[[Any], Any]]:
        ...
        """
        if val not in cls.VOCAB:
            raise KeyError(f"Variable {val} not found in MetOp MTG lexicon")
