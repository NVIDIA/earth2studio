...
class ZarrBackend:
    ...
    def __init__(
        ...
        ft: Template = Template("earth2studio_${index}"),
        ...
        self.zarr_group = zarr.api.asynchronous.open(store=zstore, mode="a")

        self._scratch_space = "zarr"
    ...
    def write(
        ...
        # Force a block waiting for zarr store to load
        # This is so we dont have multiple async calls trying to await on the same
        # File system object
        loop.run_until_complete(
                asyncio.wait_for(self.await_store(), timeout=self.async_timeout)
            )
    ...
    async def write_async(
        ...
        await self.await_store()

        # TODO: This should be async loop / gather of tasks
        for var in ds.data_vars:
            array_name = f"{self.ft.safe_substitute(index=self._index, **ft_kwargs)}_{var}"
            da = ds[var]
            # For reference on how to create then set async
            # https://zarr.readthedocs.io/en/stable/_modules/zarr/api/asynchronous.html#array
            z = await self.zarr_group.create_array(
                name=array_name,
                shape=da.shape,
                # chunks={"time": 1, "lead_time": 1}, # TODO Generalize
                dtype=da.dtype,
                dimension_names=list(da.coords.keys()),
                overwrite = True,
            )

            await z.setitem(..., da.values)
    ...
    def consolidate(
        ...
        file_name : str, optional
            Output file name, by default "consolidated.h5"
            ...
            for item in nested_iter:
                if isinstance(item, dict):
                    ...
                    if "local" in self.fs.protocol:
                        ...
                        output.append(xr.open_dataset(file, engine="h5netcdf"))
        ...
        if "local" in self.fs.protocol:
            ...
            temp_file_name = f"{hashlib.sha256(file_name.encode())}.h5"
