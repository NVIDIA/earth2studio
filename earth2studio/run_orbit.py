...
def run(
    ...
    for i in range(24):
        ...
        #time_list.append((time_in[0] + np.timedelta64(1, 'h') * (-1 * i)))
    time = to_time_array(time_list)
    p, coords = prep_data_array(
        data(time, ["cp", "lsp", "t2m", "sst"]), device=device
    )
    logger.success(f"Fetched data from {data.__class__.__name__}")

    #Add 2 components of precipitation together
    total_p_24hr = p[:,0] + p[:,1]
    #Add over time dimension
    total_p_24hr = torch.sum(total_p_24hr, dim=0)

    t2_sst_combined = torch.where(torch.isnan(p[:,3]), p[:,2], p[:,3])

    t2_max = torch.max(t2_sst_combined, dim=0).values
    t2_min = torch.min(t2_sst_combined, dim=0).values
    total_p_24hr = total_p_24hr.unsqueeze(0).unsqueeze(0)
    t2_max = t2_max.unsqueeze(0).unsqueeze(0)
    t2_min = t2_min.unsqueeze(0).unsqueeze(0)
    ...
    x, coords = orbit(x, output_coords)
    io.write(torch.squeeze(x), total_coords, output_coords["variable"])
    ...
    if plot_inference:
        ...
        img_min = np.min(data[output_coords["variable"][0]].values)
        img_max = np.max(data[output_coords["variable"][0]].values)
        plt.figure(
            figsize=(
                data[output_coords["variable"][0]].values.shape[1] / 100,
                data[output_coords["variable"][0]].values.shape[0] / 100,
            )
        )
        plt.imshow(data[output_coords["variable"][0]].values, cmap="coolwarm", vmin=img_min, vmax=img_max)
...
def check_data(x, time, in_variables):
    ...
    for i in range(len(in_variables)-4):
        ...
        ae = np.absolute(var-x.values[:,indexer])
        mae = ae.mean()
