...
class ObsGridMapping:
    ...
    def obs_coords(
        ...
        with self.cupy_context:
            ...
            if self.grid_type == "regular":
                obs_i = (obs_lat - self.grid_lat[0]) / (
                    self.grid_lat[1] - self.grid_lat[0]
                )
                obs_j = (obs_lon - self.grid_lon[0]) / (
                    self.grid_lon[1] - self.grid_lon[0]
                )
            else:
    ...
    def obs_to_grid(
        ...
        def _init_obs_mask() -> tuple[torch.Tensor, torch.Tensor]:
            y_obs = torch.zeros(
                1, *self.grid_shape, device=self.device, dtype=torch.float32
            )
            mask = torch.zeros(
                1, *self.grid_shape, device=self.device, dtype=torch.float32
            )
        ...
        obs_var = obs[
            "variable"
        ].values  # string dtype - must stay on CPU, never convert to CuPy
        with self.cupy_context:
        ...
        if len(obs_c):
            obs_val = torch.as_tensor(
                obs["observation"].values, device=self.device, dtype=torch.float32
            )
    ...
    def grid_to_obs(
        ...
        if method == "nearest":
            ...
            raise ValueError(
                f"Unknown interpolation method: {method!r}. Expected 'nearest' or 'linear'."
            )
