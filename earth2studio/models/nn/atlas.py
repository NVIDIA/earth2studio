...
try:
    ...
    import physicsnemo
    from torch_harmonics import RealSHT, InverseRealSHT, ResampleS2
    from torch_harmonics.quadrature import legendre_gauss_weights, clenshaw_curtiss_weights, lobatto_weights
...
def spherical_white_noise(shape, scale=3.56, device=None, studentt_deg=None):
    ...
    noise = (1. / math.sqrt(float(shape[-1]*shape[-2]))) * scale * noise
...
def bilinear_interpolate_torch(x, shape):
    return torch.nn.functional.interpolate(x, size=shape, 
                                            mode='bilinear', 
                                            align_corners=True, 
                                            antialias=False)
...
def _sun_ecliptic_longitude(model_time: np.ndarray) -> np.ndarray:
    ...
    mean_anomaly = np.deg2rad(
        357.52910 + 35999.05030 * julian_centuries - 0.0001559 * julian_centuries * julian_centuries - 0.00000048 * julian_centuries * julian_centuries * julian_centuries,
        dtype=np.float32,
    )

    # mean longitude
    mean_longitude = np.deg2rad(280.46645 + 36000.76983 * julian_centuries + 0.0003032 * (julian_centuries**2), dtype=np.float32)

    d_l = np.deg2rad(
        (1.914600 - 0.004817 * julian_centuries - 0.000014 * (julian_centuries**2)) * np.sin(mean_anomaly)
...
def _greenwich_mean_sidereal_time(model_time: np.ndarray) -> np.ndarray:
    ...
    theta = np.float32(67310.54841 + jul_centuries * (876600 * 3600 + 8640184.812866 + jul_centuries * (0.093104 - jul_centuries * 6.2 * 10e-6)))
...
def follmer_g(t, sigma, sigma_dot, beta, beta_dot):
    ...
    return torch.sqrt(torch.abs(2. * t * sig*((1.0 / bet) * betd * sig - sigd) - torch.pow(sig, 2)))
...
class StochasticInterpolant(torch.nn.Module):
    def __init__(self,
        ...
        if noise_sampler == 'white':
            ...
            self.noise_sampler = partial(spherical_white_noise, studentt_deg=studentt_deg)
        else:
            raise NotImplementedError()
        ...
        if sample_method == 'em':
            ...
            coeff = {'c1': [1.0/3.0, 1.0], 'beta1': [0.0, 1.0], 'beta2': [-3.0/2.0, 3.0/2.0]}
            self.sample_step = partial(self.rk_rossler_step, coeff=coeff)
        else:
            raise NotImplementedError()
    ...
    def stochastic_path(self, x0, x1, t, return_derivative=True):
        ...
        I = self.alpha(t) * x0 + self.beta(t) * x1 + self.sigma(t) * W
    ...
    def compute_drift(self, drift_velocity, x, t, cond=None, compute_normalization=False, compute_correction=False, g=None, expand_shape=None, initial=None):
        vel_inp = {'I': x, 't': t}
        if cond is not None:
    ...
    def em_step(self, drift_velocity, x, t, step, expand_shape, cond=None, compute_normalization=False, initial=None):
        ...
        drift = self.compute_drift(drift_velocity, x, previous_t, cond, compute_normalization,
                                   compute_correction, g, expand_shape, initial)
    ...
    def rk_roberts_step(self, drift_velocity, x, t, step, expand_shape, cond=None, compute_normalization=False, initial=None):
        previous_t = t[:,step].view(-1,1)
        ...
        drift_previous = self.compute_drift(drift_velocity, x, previous_t, cond, compute_normalization,
                                            compute_correction, g_previous, expand_shape, initial)

        y1 = delta_t * drift_previous + g_previous * (noise - torch.sqrt(delta_t)*S)

        drift_current = self.compute_drift(drift_velocity, x + y1, current_t, cond, compute_normalization,
                                           compute_correction, g_current, expand_shape, initial)
    ...
    def rk_rossler_step(self, drift_velocity, x, t, step, expand_shape, cond=None, compute_normalization=False, initial=None, coeff=None):
        t_n = t[:,step].view(-1,1)
        ...
        r_n_tilde = 0.5*(r_n + (1.0/math.sqrt(3.0))*torch.sqrt(delta_t)*self.noise_sampler(x.shape, device=x.device))

        drift_1 = self.compute_drift(drift_velocity, x, t_n, cond, compute_normalization,
                                     compute_correction, g_n, expand_shape, initial)
        
        h_n = x + (3.0/4.0)*delta_t*drift_1 + (3.0/2.0)*g_n_1*r_n_tilde

        drift_2 = self.compute_drift(drift_velocity, h_n, t_n_3, cond, compute_normalization,
                                     compute_correction, g_n_3, expand_shape, initial)

        x = x + (1.0/3.0)*delta_t*drift_1 + \
    ...
    def sample(self, drift_velocity, x, t=None, steps=200, cond=None, compute_normalization=False, keep_every=None, verbose=False):
        assert not ((t is None) and (steps is None))
        ...
        if keep_every is not None:
            assert steps % keep_every == 0
        
        expand_shape = [x.shape[0]] + [1]*(len(x.shape) - 1)

        x = self.sample_step(drift_velocity, x, t, 0, expand_shape, cond, compute_normalization, initial)

        kept_samples = 0
        if keep_every is not None:
            out = torch.zeros([x.shape[0], steps // keep_every] + list(x.shape[1:]), device=x.device)
        ...
        for j in range(1, t.shape[1]-1):
            ...
            x = self.sample_step(drift_velocity, x, t, j, expand_shape, cond, compute_normalization, initial)
...
class EquiangularInterpolator(nn.Module):
    ...
    def set_method(self, method):
        if method == 'conservative':
            self.interp_func = conservative_interpolate
        elif method == 'bilinear':
            self.interp_func = bilinear_interpolate
...
class NattenAttention(nn.Module):
    ...
    def __init__(
        ...
        assert kernel_size_h % 2 == 1, "kernel height size must be odd"
        assert kernel_size_w % 2 == 1, "kernel width size must be odd"
...
class NattenDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert qk_norm is False, f"{qk_norm=} is not supported"
        assert attn_mask is None, f"attn_mask is not supported"
        assert attn_drop_rate == 0.0, "attn_drop_rate > 0.0 not supported"
        assert mlp_drop_rate == 0.0, "mlp_drop_rate > 0.0 not supported"
        assert path_drop_rate == 0.0, "path_drop_rate > 0.0 not supported"
        assert not use_swiglu, "swiglu not supported"
        ...
        self.mlp = Mlp(
            in_features=hidden_size, hidden_features=mlp_hidden_size, 
...
class FourierEmbedder(nn.Module):
    ...
    def cos_sin_embedding(self, x):
        ...
        ).to(device=x.device).unsqueeze(1)
        ...
        if emb_diff != 0:
            embedding = torch.cat((embedding, 
                                   torch.zeros(embedding.shape[0], emb_diff, device=x.device)), 
                                   dim=-1)
...
class Attention(nn.Module):
    def __init__(
        ...
        if num_heads is None:
            num_heads = find_num_heads(dim, max_dim_per_head=256, divisors=64)
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_dim, num_heads=None, mlp_ratio=4.0, **block_kwargs):
        ...
        self.adaLN_modulation = torch.nn.Sequential(
            torch.nn.SiLU(),
            torch.nn.Linear(hidden_dim, 6 * hidden_dim, bias=True)
        )
...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, out_dim):
        super().__init__()
        self.norm_final = torch.nn.LayerNorm(hidden_dim, elementwise_affine=False, eps=1e-6)
...
def get_dit_block(natten=False, kernel_size=None, grid_size=None, bfloat_cast=False):
        if natten:
            assert kernel_size is not None and grid_size is not None
            block = partial(NattenDiTBlock, 
                            grid_size=grid_size, 
                            kernel_size=kernel_size)
        else:
            block = partial(DiTBlock,
...
class SInterpolantLatentDiT(physicsnemo.Module):
    def __init__(
        ...
        **kwargs
        ...
        #Patch embeddings 
        self.patch_size_1 = (self.input_shape_1_dit[0] // num_patches[0], self.input_shape_1_dit[1] // num_patches[1])
        self.patch_size_2 = (self.input_shape_2_dit[0] // num_patches[0], self.input_shape_2_dit[1] // num_patches[1])
        embed_dim = embed_dim_1 + embed_dim_2

        self.x_embedder_1 = PatchEmbed(self.input_shape_1_dit, self.patch_size_1, input_channels_1, embed_dim_1, bias=True)
        self.x_embedder_2 = PatchEmbed(self.input_shape_2_dit, self.patch_size_2, input_channels_2, embed_dim_2, bias=True)
        ...
        self.pos_embed_1 = torch.nn.Parameter(torch.zeros(1, self.x_embedder_1.num_patches, embed_dim_1), requires_grad=False)
        self.pos_embed_2 = torch.nn.Parameter(torch.zeros(1, self.x_embedder_2.num_patches, embed_dim_2), requires_grad=False)
        ...
        self.blocks = torch.nn.ModuleList([
            block(hidden_dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])

        self.final_layer = FinalLayer(embed_dim, self.patch_size_1[0] * self.patch_size_1[1] * output_channels)
    ...
    def forward(self, I, t, x_1, x_2, c_S=0.0, c_O=1.0, c_I=1.0, label=None, **kwargs):

        if isinstance(c_I, float):
            I = torch.cat((c_I*I, x_1.clone()), dim=1)
        else:
            I = torch.cat((c_I.view([c_I.shape[0]] + [1]*(len(x_1.shape)-1))*I, x_1.clone()), dim=1)
        ...
        #Embeddings
        I = self.x_embedder_1(I) + self.pos_embed_1
...
def validate_patch_size(input_size, patch_size=None, n_patch=None):
    ...
    if patch_size is None and n_patch is None:
        raise ValueError(f'You passed both patch_size=None and n_patch=None. One should be defined as a tuple of ints.')

    if patch_size is not None and n_patch is not None:
        raise ValueError(f'You passed both {patch_size=} and {n_patch=}. Only one be defined, as a tuple of ints.')

    if patch_size is None:
        patch_size = _ensure_int_tuple(patch_size, 2)

        patch_size = (input_size[0]//n_patch[0] + bool(input_size[0] % n_patch[0]),
                      input_size[1]//n_patch[1] + bool(input_size[1] % n_patch[1]))
    else:
        n_patch = _ensure_int_tuple(n_patch, 2)

        n_patch = (input_size[0]//patch_size[0] + bool(input_size[0] % patch_size[0]),
...
class PatchPad(nn.Module):
    def __init__(self, input_size, target_size):
        ...
        self.lat_pad, self.lon_pad = abs(target_size[0] - input_size[0]), abs(target_size[1] - input_size[1])
...
class PatchUnpad(nn.Module):
    def __init__(self, input_size, target_size):
        ...
        if input_size[0] < target_size[0]:
            raise ValueError(f'Input_size[0] {input_size} is smaller than target_size[0] {target_size}, cannot unpad.')
        if input_size[1] < target_size[1]:
            raise ValueError(f'Input_size[1] {input_size} is smaller than target_size[1] {target_size}, cannot unpad.')
...
class NattenCombineDiT(physicsnemo.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        output_channels = output_channels if output_channels is not None else input_channels1
        self.output_channels = output_channels
        ...
        patch_size, n_patch, latent_size = validate_patch_size(input_size=input_size1, patch_size=patch_size, n_patch=n_patch)
        ...
        if patch_processing == 'pad':
            ...
            self.preprocess1 = PatchResample(target_size=latent_size, add_conv=patch_processing_add_conv,
                                            input_channels=input_channels1, output_channels=input_channels1)
            self.postprocess = PatchResample(target_size=input_size1, add_conv=patch_processing_add_conv,
                                            input_channels=output_channels, output_channels=output_channels)
        self.x_embedder1 = PatchEmbed(latent_size, patch_size, input_channels1, hidden_channels1, bias=True)
        self.pos_embed1 = nn.Parameter(torch.zeros(1, self.x_embedder1.num_patches, hidden_channels1), requires_grad=False)
        ...
        patch_size2, _, latent_size = validate_patch_size(input_size=input_size2, patch_size=None, n_patch=n_patch)
        self.input_size2 = input_size2
        if patch_processing == 'pad':
            self.preprocess2 = PatchPad(input_size2, target_size=latent_size)
        elif patch_processing == 'resample':
            self.preprocess2 = PatchResample(target_size=latent_size, add_conv=patch_processing_add_conv,
                                            input_channels=input_channels2, output_channels=input_channels2)
        self.x_embedder2 = PatchEmbed(latent_size, patch_size2, input_channels2, hidden_channels2, bias=True)
        self.pos_embed2 = nn.Parameter(torch.zeros(1, self.x_embedder2.num_patches, hidden_channels2), requires_grad=False)

        # Combine the two embeddings: results in hidden-channels sum of the two 
        if self.combination_mode == 'token_concatenation' or self.combination_mode == 'token_addition' or self.combination_mode == 'token_multiplication':
        ...
        # self.t_embedder = TimestepEmbedder(hidden_channels)
        self.t_embedder = FourierEmbedder(hidden_channels)
        if date_condition:
            self.date_embedder = DateEmbedder(hidden_channels)
        else:
            self.date_embedder = None

        # self.blocks = nn.ModuleList([
        #     DiTBlock(hidden_channels, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        # ])
        block = self._get_dit_block(use_natten, natten_kernel_size)
        self.blocks = nn.ModuleList([
            block(hidden_size=hidden_channels, num_heads=num_heads, mlp_ratio=mlp_ratio) 
            for _ in range(depth)
        ])
        self.final_layer = FinalLayer(hidden_channels, patch_size[0] * patch_size[1] * output_channels)
    ...
    def _get_dit_block(self, use_natten: bool, 
                       natten_kernel_size: Optional[Tuple[int, int]]=None):
    ...
    def forward(self, x_1, x_2, t=None, date=None, **kwargs):
        ...
        x_1 = self.x_embedder1(x_1) + self.pos_embed1  # (N, seq_length, seq_dim) -> (N, seq_length, hidden_dim)
        x_2 = self.x_embedder2(x_2) + self.pos_embed2  # (N, seq_length, seq_dim) -> (N, seq_length, hidden_dim)

        # x = torch.cat((x_1, x_2), dim=-1)

        #################################################
        # Recombination with x_t
        if self.combination_mode == 'token_addition' and self.hidden_channels1 == self.hidden_channels2:
            x = x_1 + x_2               
        elif self.combination_mode == 'token_multiplication' and self.hidden_channels1 == self.hidden_channels2:
            x = x_1 * x_2   
        elif self.combination_mode == 'token_concatenation':
            x = torch.cat((x_1, x_2), dim=1)      
        elif self.combination_mode == 'channel_concatenation' and x_1.shape[1] == x_2.shape[1]:
            x = torch.cat((x_1, x_2), dim=-1)            
        ...
        for j, block in enumerate(self.blocks):
            if self.checkpoint is not None and (j + 1) % self.checkpoint == 0:
                x = torch.utils.checkpoint.checkpoint(block, x, conditioning, use_reentrant=False)
            else:
...
class BaseProcessor(physicsnemo.Module):
    def __init__(
        self, 
        normalization='gaussian', normalizer_stats_shape=None, normalizer_eps=1e-6, normalizer_range=[-1, 1],
        positional_encoding=None,
        static_channels_shape=None,
        cosine_zenith_angle=False,
        input_grid_shape=(721, 1440),
        regrid_to_healpix: bool = False, healpix_level: int = 8,
        device=None,
    ):
        super().__init__()
        ...
        if normalization is None:
            ...
            self.normalizer_in = GaussianNormalizer(stats_shape=normalizer_stats_shape, eps=normalizer_eps)
            self.normalizer_out = GaussianNormalizer(stats_shape=normalizer_stats_shape, eps=normalizer_eps)
        elif normalization.lower() == 'range':
            self.normalizer_in = RangeNormalizer(stats_shape=normalizer_stats_shape, range=normalizer_range)
            self.normalizer_out = RangeNormalizer(stats_shape=normalizer_stats_shape, range=normalizer_range)
        else:
            raise ValueError(f'normalization has to be one of ["gaussian", "range"], but got {normalization=}')

        if cosine_zenith_angle:
            lon = np.linspace(0, 2*np.pi, input_grid_shape[1] + 1, dtype=np.float32)[:-1]
            lat = np.linspace(-np.pi/2, np.pi/2, input_grid_shape[0], dtype=np.float32)
    ...
    def add_cosine_zenith_angle(self, x, x_dates):
        if not isinstance(x_dates, np.ndarray):
            if x.shape[0] == 1:
                ...
                raise ValueError(f'Got a single date for a batch of {x.shape[0]} samples')
        cos_zenith_angles = [torch.from_numpy(cos_zenith_angle(date, self.lon_grid, self.lat_grid)) for date in x_dates]
    ...
    def add_static_channels(self, x):
        if self.static_channels is not None:
            return torch.cat((x, self.static_channels.expand(x.shape[0], -1, -1, -1)), dim=1)
        return x
...
class SinterpolantDifferenceProcessor(BaseProcessor):
    def __init__(
        ...
        super().__init__(normalizer_stats_shape,
                         normalizer_eps,
