...
class Conv2d(torch.nn.Module):
    def __init__(
        ...
        if up and down:
            raise ValueError("up and down can't both be True.")
        super().__init__()
...
class SongUNetRegression(torch.nn.Module):
    def __init__(
        ...
        embedding_type="zero",  # Timestep embedding type: 'positional' for DDPM++, 'fourier' for NCSN++.
        ...
        assert embedding_type in ["zero"]  # noqa: S101
        assert encoder_type in ["standard", "skip", "residual"]  # noqa: S101
        assert decoder_type in ["standard", "skip"]  # noqa: S101

        super().__init__()
        self.label_dropout = label_dropout
        self.emb_channels = model_channels * channel_mult_emb
        noise_channels = model_channels * channel_mult_noise  # noqa
...
class SongUNet(torch.nn.Module):
    def __init__(
        ...
        assert embedding_type in ["fourier", "positional"]  # noqa: S101
        assert encoder_type in ["standard", "skip", "residual"]  # noqa: S101
        assert decoder_type in ["standard", "skip"]  # noqa: S101
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(
        ...
        **model_kwargs,
        ...
        assert F_x.dtype == dtype  # noqa: S101
...
class EasyRegression(torch.nn.Module):
    ...
    def __init__(self, model, latent_shape):
        super().__init__()
    ...
    def forward(
        ...
        latent = torch.zeros(
            [condition.shape[0], *self.latent_shape], device=condition.device
        )
...
class RegressionWrapperV2(torch.nn.Module):
    ...
    def forward(
        ...
        **model_kwargs,
        ...
        dtype = (
            torch.float16
            if (self.use_fp16 and not force_fp32 and sigma.device.type == "cuda")
            else torch.float32
        )
