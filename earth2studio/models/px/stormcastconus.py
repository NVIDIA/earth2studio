...
class StormCastCONUS(torch.nn.Module, AutoModelMixin, PrognosticMixin):
    ...
    Badges
    ------
    region:na task:nowcasting product:wind product:temp product:precip product:radar product:atmos year:2026 gpu:24gb
    ...
    def __init__(
        ...
        if (hrrr_lat_lim, hrrr_lon_lim) != FULL_MODEL_HRRR_BBOX:
            if not isinstance(self.diffusion_model, _SplitModelWrapper):
                raise ValueError(
                    "To crop the model to a subdomain, diffusion_model must be _SplitModelWrapper."
                )
