...
from earth2studio.models.da.utils import ObsGridMapping
...
class StormCastCONUS(torch.nn.Module, AutoModelMixin, PrognosticMixin):
    ...
    def __init__(
        ...
        if (hrrr_lat_lim, hrrr_lon_lim) != FULL_MODEL_HRRR_BBOX:
            ...
            if (hrrr_lat_lim[0] - FULL_MODEL_HRRR_BBOX[0][0]) % p[0]:
                raise ValueError(f"hrrr_lat_lim[0] - {FULL_MODEL_HRRR_BBOX[0]} must be divisible by {p[0]}")
            if (hrrr_lat_lim[1] - hrrr_lat_lim[0]) % p[0]:
                raise ValueError(f"hrrr_lat_lim[1] - hrrr_lat_lim[0] must be divisible by {p[0]}")
            if (hrrr_lon_lim[0] - FULL_MODEL_HRRR_BBOX[1][0]) % p[1]:
                raise ValueError(f"hrrr_lon_lim[0] - {FULL_MODEL_HRRR_BBOX[1]} must be divisible by {p[1]}")
            if (hrrr_lon_lim[1] - hrrr_lon_lim[0]) % p[1]:
                raise ValueError(f"hrrr_lon_lim[1] - hrrr_lon_lim[0] must be divisible by {p[1]}")

            self.diffusion_model.crop_model((hrrr_lat_lim, hrrr_lon_lim))

        hrrr_lat, hrrr_lon = HRRR.grid()
        self.lat = hrrr_lat[hrrr_lat_lim[0]:hrrr_lat_lim[1], hrrr_lon_lim[0]:hrrr_lon_lim[1]]
        self.lon = hrrr_lon[hrrr_lat_lim[0]:hrrr_lat_lim[1], hrrr_lon_lim[0]:hrrr_lon_lim[1]]
        ...
        self.register_buffer("cond_mean_inv_std", -self.conditioning_means / self.conditioning_stds)
    ...
    def compile_model(self) -> None:
        ...
        self.diffusion_model.forward = torch.compile(self.diffusion_model.forward, dynamic=False)

    def normalize(self, x: torch.Tensor, in_place: bool = False) -> torch.Tensor:
        """Normalise ``x`` to zero mean, unit variance."""
        return torch.addcmul(self.mean_inv_std, x, self.inv_std, out=x if in_place else None)
    ...
    def _forward(
        ...
        """
        year_phase = 2 * np.pi * ((time - time.astype("datetime64[Y]")).astype("timedelta64[h]") / np.timedelta64(8766, 'h'))
        scalar_conds = torch.as_tensor([np.sin(year_phase), np.cos(year_phase)]).to(device=x.device, dtype=x.dtype).repeat(x.shape[0], 1)
        time_dt = datetime.fromisoformat(str(time))
        cos_zen = cos_zenith_angle(time_dt, self.lon_tensor, self.lat_tensor).expand(x.shape[0], 1, -1, -1)
        ...
        condition = TensorDict({"cond_concat": cond, "cond_vec": scalar_conds}, device=x.device)
    ...
    def _conditioned_x0_predictor(
        ...
        def x0_predictor(x_noisy: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
            with torch.autocast(x_noisy.device.type, dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            for k, lead_time in enumerate(coords["lead_time"]):
                ...
                for i0 in range(0, len(coords["batch"]), self.batch_size):
                    ...
                    x[i0:i1, j, k] = self._forward(
                        x[i0:i1, j, k], conditioning[i0:i1, j, k], t, y_obs=y_obs, mask=mask
                    )
...
class SplitModelWrapper(torch.nn.Module):
    ...
    MIN_VALUES: dict[str, float] = {
        v: 0.0
        for v in VARIABLES if any(v.startswith(c) for c in ["p", "Z", "t", "q"])
    }
    MIN_VALUES["msl"] = 0.0
    ...
    def __init__(
        ...
        self.pos_embed_full = {k: model.model.model.tokenizer.pos_embed for (k, model) in self.models.items()}
    ...
    def crop_model(self, bbox: tuple[tuple[int, int], tuple[int, int]]) -> None:
        ...
        (H_full, W_full) = (self.full_grid_shape[0] // p[0], self.full_grid_shape[1] // p[1])
        bb_crop = ((bbox[0][0] // p[0], bbox[0][1] // p[0]), (bbox[1][0] // p[1], bbox[1][1] // p[1]))

        def _crop_single_posemb(pos_embed_full: torch.Tensor) -> torch.Tensor:
            pos_embed_full_2D = pos_embed_full.view(H_full, W_full, pos_embed_full.shape[-1])
            pos_embed_crop_2D = pos_embed_full_2D[bb_crop[0][0] : bb_crop[0][1], bb_crop[1][0] : bb_crop[1][1], :]
        ...
        for (key, model) in self.models.items():
            ...
            dit.tokenizer.register_parameter("pos_embed", torch.nn.Parameter(pos_embed_crop))
    ...
    def forward(
        ...
        **model_kwargs
        ...
        if sigma[0] >= self.sigma_threshold:
            ...
            for split in ["tq", "uv"]:
                y_split = self.models[f"{split}_low"](x, sigma, condition, **model_kwargs)
