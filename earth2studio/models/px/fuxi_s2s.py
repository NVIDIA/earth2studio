...
class FuXiS2S(torch.nn.Module, AutoModelMixin, PrognosticMixin):
    ...
    Example
    -------
    The following uses hourly ARCO ERA5 to prepare the two complete UTC
    daily means expected by FuXi-S2S.
    ...
    # Prepare hourly ERA5 as complete UTC daily means on the model grid
    data = FuXiS2SERA5(ARCO_ERA5())

    # Load the model and run one daily step
    package = FuXiS2S.load_default_package()
    model = FuXiS2S.load_model(package).to("cuda:0")
    io = run.deterministic(["2024-01-15"], 1, model, data, ZarrBackend())
    ```
