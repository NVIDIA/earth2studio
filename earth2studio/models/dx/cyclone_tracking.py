...
class CycloneTrackingVorticity(torch.nn.Module, AutoModelMixin):
    ...
    def __call__(
        ...
        for i in range(x.shape[0]):
            ...
            centers = find_centers_wuduan(lat, lon, vort850, w10m, msl)
            outs.append(centers)
...
def find_centers_vitart(
    ...
    vlm_loc = torch.stack((lat[vlm[:, 0], vlm[:, 1]], lon[vlm[:, 0], vlm[:, 1]]), dim=1)

    # Get local maximum of average temperature between 500 and 200mb
    tlm = get_local_max(t_200_500_mean, min_distance=10, exclude_border=exclude_border)
    tlm_loc = torch.stack((lat[tlm[:, 0], tlm[:, 1]], lon[tlm[:, 0], tlm[:, 1]]), dim=1)
...
def find_centers_wuduan(
    ...
    for prop in props:
        ...
        solidity_center = bool_center.sum() / convex_hull_image(bool_center.get()).sum()
        ...
        if mean_inner > mean_outer:
            ...
            centers.append(
                torch.as_tensor([lat_, lon_, msl_min, w10m_max], device=vort850.device)
            )
