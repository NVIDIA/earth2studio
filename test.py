...
def _normalize_to_noon(dt: datetime) -> datetime:
...
data = model.create_time_window_wrapper(data, time_fn=_normalize_to_noon)


time_arr = np.array([datetime(2037, 9, 6)], dtype=np.datetime64)
x, coords = fetch_data(
    source=data,
    time=time_arr,
...
print(x, coords["variable"], list(coords.keys()))
