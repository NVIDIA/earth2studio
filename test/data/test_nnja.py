...
def test_nnja_obs_conv_finalize_decoded_df():
    ...
    # Empty rows
    result = ds._finalize_decoded_df([], ["t"], convert_pres_mb_to_pa=True)
    assert result.empty
    ...
    rows = [
        {
            ...
            "station_elev": 1000.0,
            "observation": 273.15,
            "variable": "other_var",  # Not among requested variables
        }
    ]
    result = ds._finalize_decoded_df(rows, ["t"], convert_pres_mb_to_pa=True)
    ...
    result = ds._finalize_decoded_df(rows_match, ["t"], convert_pres_mb_to_pa=True)
    assert len(result) == 1
    ...
    result_no_conv = ds._finalize_decoded_df(
        rows_match, ["t"], convert_pres_mb_to_pa=False
    )
...
def test_nnja_obs_conv_finalize_gpsro_preserves_pressure_and_elevation_units():
    ...
    result = ds._finalize_decoded_df(rows, ["gps"], convert_pres_mb_to_pa=False)
...
def test_nnja_obs_conv_finalize_pres_filters_level_cat():
    ...
    result = ds._finalize_decoded_df(rows, ["pres"], convert_pres_mb_to_pa=True)
