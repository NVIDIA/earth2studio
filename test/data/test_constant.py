...
from earth2studio.data import Constant, Constant_FX


@pytest.mark.parametrize("value", [1, 2, 1.6])
...
def test_constant(value, time, variable, lat, lon):

    coords = OrderedDict({"lat": lat, "lon": lon})

    data_source = Constant(coords, value)
    ...
    assert np.all(data.values == value)


@pytest.mark.parametrize("value", [-0.5, 42])
@pytest.mark.parametrize(
    "time",
    [
        datetime.datetime.now(),
...
@pytest.mark.parametrize("lat", [[-1, 0, -1], np.linspace(-90, 90, 181)])
@pytest.mark.parametrize("lon", [[0, 1, 2, 3], np.linspace(0, 359, 360)])
def test_random_forecast(value, time, lead_time, variable, lat, lon):

    coords = OrderedDict({"lat": lat, "lon": lon})

    data_source = Constant_FX(coords, value)
