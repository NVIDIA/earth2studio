...
class TestMSCObjectStorageAzure:
    ...
    def test_init_azure_managed_identity_with_account_name(self, mock_msc):
        ...
        assert storage.endpoint_url == "https://myaccount.blob.core.windows.net"

    def test_init_azure_derived_url_ignores_endpoint_url_kwarg(self, mock_msc):
        """Azure blob URL is always derived from azure_account_name; endpoint_url is unused."""
        storage = MSCObjectStorage(
            bucket="mycontainer",
            storage_type="azure",
            azure_account_name="myaccount",
            endpoint_url="http://ignored-custom:10000",
        )
        assert storage.endpoint_url == "https://myaccount.blob.core.windows.net"

    def test_init_azure_no_account_name_raises(self, mock_msc):
        """Azure raises ObjectStorageError when azure_account_name is missing."""
        with pytest.raises(ObjectStorageError, match="requires azure_account_name"):
            MSCObjectStorage(
                bucket="mycontainer",
                storage_type="azure",
            )
