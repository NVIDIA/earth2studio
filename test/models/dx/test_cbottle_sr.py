...
class TestCBottleSRMock:
    ...
    def test_cbottle_sr_healpix_exceptions(self, mock_cbottle_core_model):
        ...
        with pytest.raises(
            ValueError,
            match="super_resolution_window is not supported with healpix output_type",
        ):
            CBottleSR(
                mock_cbottle_core_model,
                output_type="healpix",
    ...
    @pytest.mark.parametrize("device", ["cuda:0"])
    def test_cbottle_sr_healpix_coordinate_validation(
        self, device, mock_cbottle_core_model
    ):
...
def test_cbottle_sr_package_healpix(
    device,
    input_type,
    output_type,
