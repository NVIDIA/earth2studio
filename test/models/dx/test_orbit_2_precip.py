...
class PhooORBIT2Precip(torch.nn.Module):
    ...
    def forward(self, x, in_variables, out_variables):
        x_out = torch.zeros(x.shape[0],len(out_variables), self.lat_out, self.lon_out)
        return x_out

@pytest.mark.parametrize(
    "x",
    [
        torch.randn(1, 19, 721, 1440),
        torch.randn(2, 19, 721, 1440),
    ],
)
...
def test_orbit2_precip(x, device):
    ...
    normalize_mean_lowres = {'land_sea_mask': np.array([0.33852854], dtype=np.float32), 
    ...
    normalize_std_lowres = {'land_sea_mask': np.array([0.46153313], dtype=np.float32), 
    ...
    dx = OrbitGlobalPrecip(model, land_sea_mask, orography, lattitude, landcover, normalize_mean_lowres_npz, normalize_std_lowres_npz, normalize_mean_highres_npz, normalize_std_highres_npz, do_tiling, div, overlap).to(device)
...
def test_orbit2_precip_package(x, device, model_size):
    ...
    dx = OrbitGlobalPrecip.load_model(package, "global", model_size, "precipitation").to(device)
