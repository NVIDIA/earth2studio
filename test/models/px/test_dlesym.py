...
def test_dlesym_forward(device, grid_type, batch_size):
    ...
    output_vars = tensor_output_coords(model, tensor_input_coords(model))["variable"]
    ...
    expected_coords = tensor_output_coords(model, in_coords)
    assert output.shape == (
        batch_size,
        len(time),
...
def test_dlesym_latlon_regridding(device, batch_size):
    ...
    ll_coords = tensor_input_coords(model)
