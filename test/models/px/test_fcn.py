...
def test_fcn_coordinate_signatures():
    ...
    assert input_signature.data.nbytes == output_signature.data.nbytes == 0
    assert (
        input_signature.e2s.get_grid().fingerprint()
        == output_signature.e2s.get_grid().fingerprint()
    )
    assert output_signature.coords["lead_time"] == np.timedelta64(6, "h")
