...
def model_domain(request, monkeypatch):
    ...
    if request.node.originalname == "test_atlas_iter":
        ...
        monkeypatch.setattr(Atlas, "input_coords", small)
    ...
    if request.node.originalname in (
        ...
        monkeypatch.setattr(Atlas, "__init__", inspect.unwrap(Atlas.__init__))
