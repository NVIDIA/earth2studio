...
try:
    from physicsnemo.utils.insolation import insolation as _pnm_insolation
except Exception:
    _pnm_insolation = None

_needs_insolation = pytest.mark.skipif(
    _pnm_insolation is None,
    reason="StormScopeNSRDB requires physicsnemo.utils.insolation",
)
...
def create_spoof_nsrdb_model(
    ...
    """Create a spoof StormScopeNSRDB (GHI estimator) for testing.

    The model is a same-time estimator conditioned on GOES: it has a single
    ``ghi`` output variable and uses a spoof diffusion denoiser.
    """
    y = np.arange(h)
    x = np.arange(w)
    lat = torch.linspace(25, 50, h).unsqueeze(1).repeat(1, w)
    ...
    # Static invariants: [sin(lat), cos(lat), sin(lon), cos(lon)]
...
@pytest.mark.parametrize(
    "time",
    [
        np.array([np.datetime64("2024-07-15T18:00")]),
        np.array(
            [
                np.datetime64("2024-07-15T18:00"),
                np.datetime64("2024-06-04T20:00"),
            ]
        ),
    ],
)
@pytest.mark.parametrize("device", ["cpu", "cuda:0"])
@pytest.mark.parametrize("batch", [1, 2])
def test_stormscope_nsrdb_estimate_from_goes(time, device, batch):
    """Test StormScopeNSRDB GHI estimation from GOES conditioning."""
    nvar_cond = 8
    h, w = 32, 64

    model = create_spoof_nsrdb_model(
        nvar_cond=nvar_cond, h=h, w=w, device=device, partial_mask=True
    )

    # Build GOES conditioning on the model's native grid
    dc = OrderedDict([("y", model.y), ("x", model.x)])
    r = Random(dc)
    lead_time = model.input_coords()["lead_time"]
    conditioning, conditioning_coords = fetch_data(
        r, time, model.conditioning_variables, lead_time, device=device
    )
    conditioning = conditioning.unsqueeze(0).repeat(batch, 1, 1, 1, 1, 1)
    conditioning_coords.update({"batch": np.arange(batch)})
    conditioning_coords.move_to_end("batch", last=False)

    out, out_coords = model.estimate_from_goes(conditioning, conditioning_coords)

    # Output is a single ghi variable on the native grid
    assert out.shape == torch.Size([batch, len(time), 1, 1, h, w])
    assert (out_coords["variable"] == np.array(["ghi"])).all()
    assert np.all(out_coords["time"] == time)
    handshake_dim(out_coords, "x", 5)
    handshake_dim(out_coords, "y", 4)
    handshake_dim(out_coords, "variable", 3)
    ...
    # Invalid (masked) pixels must be NaN; valid pixels finite and non-negative
    valid = model.valid_mask.cpu().numpy()
    out_np = out.detach().cpu().numpy()
    assert np.all(np.isnan(out_np[..., ~valid]))
    finite = out_np[..., valid]
...
@_needs_insolation
@pytest.mark.parametrize("device", ["cpu", "cuda:0"])
def test_stormscope_nsrdb_output_coords(device):
    ...
    assert in_coords["y"].shape == (h,)
    assert in_coords["x"].shape == (w,)

    # Build a valid 6D input coordinate system to exercise output_coords handshakes
    coords = OrderedDict(
        {
            "batch": np.arange(1),
            "time": np.array([np.datetime64("2024-07-15T18:00")]),
            "lead_time": model.input_times,
            "variable": np.array(["ghi"]),
            "y": model.y,
            "x": model.x,
        }
    )
    out_coords = model.output_coords(coords)
    expected_lead = model.output_times + coords["lead_time"][-1]
    assert np.array_equal(out_coords["lead_time"], expected_lead)
    assert (out_coords["variable"] == np.array(["ghi"])).all()

    # Mismatched grid size should fail the handshake
    bad_coords = coords.copy()
    bad_coords["y"] = np.arange(h + 1)
