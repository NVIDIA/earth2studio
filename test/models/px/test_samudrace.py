...
def build_input(model, time, batch=1):
    ...
    torch.manual_seed(1)
    in_coords = tensor_input_coords(model)
...
def test_samudrace_input_coords(model):
    ...
    out_coords = tensor_output_coords(model, tensor_input_coords(model))
...
def test_samudrace_iter(model, batch):
    ...
    var_list = list(tensor_output_coords(model, coords.copy())["variable"])
...
def test_samudrace_forcing_window_from_file(model, tmp_path):
    ...
    var_list = list(tensor_output_coords(p, coords.copy())["variable"])
    for out in outputs:
...
def test_samudrace_package():
    ...
    in_coords = tensor_input_coords(model)
    out_vars = list(tensor_output_coords(model, tensor_input_coords(model))["variable"])
