...
For example, a {math}`10~\mathrm{K}` change in
...
constant over a short interval and thus, by equation {eq}`eq:log_p_deriv`, so is
...
Where {math}`T_\mathrm{m} = T(\Phi_\mathrm{m})`. Plugging this into equation {eq}`eq:log_p_deriv`
...
the Taylor series of the inverse of equation {eq}`eq:lapse_rate` at {math}`\Phi_\mathrm{m}`:
...
The first term on the right-hand side is identical to equation {eq}`eq:log_p_deriv`, so we
can compute it using the log-linear interpolation of equation {eq}`eq:log_linear_interp` and
...
Comparing this to equation {eq}`log_p_deriv_corr` shows that we should have:
...
Applying the correction from equation {eq}`log_p_corr` using the average lapse
...
{math}`-9.9 \times 10^{-3}\ \mathrm{K\,m^{-1}}`. This is within the physical range of
