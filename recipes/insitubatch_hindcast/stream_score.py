...
def score_window(model, x_all, coords_all, leads_h, nsteps, acc):
    ...
    ic_coords = OrderedDict(
        [
            ("time", coords_all["time"]),
            ("lead_time", coords_all["lead_time"][0:1]),
...
def run_insitu(variables, start, n_init, leads_h, nsteps, batch_size):
    ...
    for (
        x_all,
        coords_all,
    ) in feed:  # one batch per window (stream) or the whole campaign (dense)
...
def run_e2s(variables, start, n_init, leads_h, nsteps):
    ...
    times = decode_cf_time(
        np.asarray(g["time"][:]), attrs["units"], attrs.get("calendar", "standard")
    )
    inits = times[start : start + n_init]
    feed = build_feed(
        variables, start, n_init, leads_h, batch_size=n_init
    )  # for coords/model only
    model = make_model(variables, feed)
    feed.dataset.close()
    ...
    dense = np.empty(
        (n_init, len(leads_h), len(variables), len(lat), len(lon)), dtype=np.float32
    )
    for i, it in enumerate(inits):
        # Realistic per-init verification fetch: all leads for this init in one call.
        valids = [
            (np.datetime64(it) + np.timedelta64(h, "h"))
            .astype("datetime64[s]")
            .astype("O")
            for h in leads_h
        ]
    ...
    coords_all = OrderedDict(
        [
            ...
            (
                "lead_time",
                np.array(
                    [np.timedelta64(h, "h") for h in leads_h], dtype="timedelta64[ns]"
                ),
            ),
...
def main():
    ...
    rmse = acc.table()
    print(
        f"mode={args.mode}  grid={args.n_init}x{args.n_leads}x{len(args.vars)}  window={args.window}"
    )
