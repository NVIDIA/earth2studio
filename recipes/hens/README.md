...
2. **Configure the Reproduction Run**
   ...
   - If the original run didn't specify a random seed, you can find it in the
     `random_seed` attribute of the field output files. Each of the four IO
     backends this recipe supports (`KVBackend`, `XarrayBackend`, `ZarrBackend`,
     `NetCDF4Backend`) records it together with `batch_ids`, `model_package`,
     `nensemble`, `batch_size` and `torch_version`
     <!-- - The cyclone tracker file TODO: add to file-->
   - These attributes record how the random seed was created. To reproduce the
     forecast, the rest of the configuration must also match, including
     `perturbation` and `data_source`, which are not stored in the output
...
For each supported IO backend, metadata is added after processing each combination
of model checkpoint and initial condition. This metadata includes `random_seed`,
`batch_ids`, `model_package`, `nensemble`, `batch_size`, and `torch_version`. For
`KVBackend` and `XarrayBackend`, these values become attributes of the netCDF file.
For `ZarrBackend` and `NetCDF4Backend`, whose data is already on disk, they are added
directly to the existing output.

Setting `thread_io: True` writes `KVBackend` and `XarrayBackend` output in the
background so that the next forecast can start sooner. It has no effect on
`ZarrBackend` or `NetCDF4Backend`, which write data during inference. All configurations
in `cfg/` use one of these two backends.

```yaml
file_output:
    path: 'outputs'  # directory to which files are written
    output_vars: ["t2m", 'u10m', 't850', 'z500'] # variables to write out
    thread_io: False      # export in-memory backends on a writer thread pool
