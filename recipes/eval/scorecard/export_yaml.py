...
"""Export the scorecard YAML (eval_scores_<model>.yaml) from a scored run.

The YAML is the hand-off between this recipe (which needs a GPU cluster and
...
own ``src.report.aggregation`` -- nothing is reimplemented here.
    ...
    python export_yaml.py fcn3 aurora          # -> exports/eval_scores_<model>.yaml
    python export_yaml.py fcn3 --docs          # also copy into docs/_static/scorecard/
"""
...
def provenance(run: Path) -> dict:
    ...
    # torch.__version__ is a str subclass yaml.safe_dump refuses; normalise.
...
def main() -> int:
    ...
    ap.add_argument(
        "--docs",
        action="store_true",
        help=f"also copy each YAML to {DOCS_STATIC}/",
    )
    args = ap.parse_args()
    ...
    for model in args.models:
        ...
        # JSON body under a .yaml name: JSON is a YAML subset, so this stays a
        # perfectly ordinary YAML file for humans and yaml.safe_load, while
        # the docs plot can parse the SAME file with the browser's built-in
        # JSON.parse -- no data embedded in HTML, no JS YAML library.
        text = (
            "# Earth2Studio scorecard export -- generated by "
            "export_yaml.py, do not hand-edit.\n"
            + json.dumps(export(model, run), indent=1, ensure_ascii=False)
            + "\n"
        )
        out = EXPORTS / f"eval_scores_{model}.yaml"
        out.write_text(text)
        print(f"wrote {out}  ({out.stat().st_size / 1e3:.0f} kB)")
        if args.docs:
            DOCS_STATIC.mkdir(parents=True, exist_ok=True)
            dst = DOCS_STATIC / f"eval_scores_{model}.yaml"
            dst.write_text(text)
            print(f"  ->  {dst}")
    return 0
