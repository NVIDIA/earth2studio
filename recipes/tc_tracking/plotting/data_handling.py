...
def match_tracks(pred_tracks, true_track):
    ...
    for _pred_track_dict in pred_tracks:
        ...
        for ii in range(n_tracks):
            ...
            lat_true = true_track.loc[true_track["time"] == track["time"].iloc[0]][
                "lat_ib"
            ].item()
            lon_true = true_track.loc[true_track["time"] == track["time"].iloc[0]][
                "lon_ib"
            ].item()
...
def extract_tracks_from_file(csv_file):
    ...
    times = [
        pd.to_datetime(
            f'{tracks["year"].iloc[jj]}-{int(tracks.iloc[jj]["month"]):02d}-{int(tracks.iloc[jj]["day"]):02d} {int(tracks.iloc[jj]["hour"]):02d}:00:00'
        )
...
def get_ensemble_averages(
    pred_tracks, tru_track, vars: list[str] = ["msl", "wind_speed", "x", "y", "z"]
):
