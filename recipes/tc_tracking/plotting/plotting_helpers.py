...
def plot_spaghetti(
    ...
    lat_min, lat_max, lon_min, lon_max = 90, -90, 360, -0.1
...
def plot_relative_over_time(
    ...
    plt.xlim(
        track["time"].min() - np.timedelta64(6, "h"),
...
def plot_over_time(
    ...
    for ii in range(_ax.shape[0]):
        for _track in pred_tracks:
            ...
            _ax[ii].plot(track["time"], yy, color="black", alpha=0.1)
...
def plot_ib_era5(
    tru_track, case, vars=["msl", "wind_speed"], out_dir: str | None = None
):
    plt.close("all")
...
def plot_extreme_extremes_histograms(
    ...
    fig.suptitle(
        f'{(case.split("_")[0]).upper()} initialised on {pred_tracks[0]["ic"]}',
        fontsize=16,
    )
