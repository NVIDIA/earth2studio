...
from earth2studio.statistics import lat_weight, crps, mean, variance
from earth2studio.utils.type import TimeArray, VariableArray


def open_ds(fl):
    if fl.endswith('.zarr') or (os.path.isdir(fl) and os.path.exists(os.path.join(fl, '.zgroup'))):
...
def compute_mse(ds):
    ...
    for ivar, var in enumerate(vars):
        ...
        se = torch.zeros(len(var_ds.time.values), len(var_ds.lead_time.values),
                         len(var_ds.lat.values),  len(var_ds.lon.values))

        for ii, ic in enumerate(var_ds.time.values):
            print(f'processing time {ic} ({ii+1}/{len(var_ds.time.values)})', end='\r', flush=True)
        ...
        weights = torch.as_tensor(lat_weight(np.linspace(ds.lat[-1], ds.lat[0], len(ds.lat)))).unsqueeze(1).repeat(1, 1440)
...
def compute_crps(ds):
    ...
    _crps = crps(ensemble_dimension='ensemble',
    ...
    for ivar, var in enumerate(vars):
        ...
        for ii, ic in enumerate(ds.time.values):
            print(f'processing time {ic} ({ii+1}/{len(ds.time.values)})', end='\r', flush=True)

            tru, tru_coords = fetch_data(_truth,
                                         time=[ic],
            ...
            _ccc, _ = _crps(x=pred.to(DistributedManager().device),
...
def score(fl):
    ...
    if False:
        ds = ds.sel(time=ds.time[:5]).sel(ensemble=ds.ensemble[:50]).sel(lead_time=ds.lead_time[:10])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("Example: python score_results.py score_2020_2020-01-01T00.00.00_mems0000-0049.nc")
        exit()
