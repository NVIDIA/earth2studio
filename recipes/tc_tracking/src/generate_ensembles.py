...
from src.data.file_output import setup_output, initialise_netcdf_output, write_to_store
from src.utils import run_with_rank_ordered_execution, set_initial_times, remove_duplicates, get_set_of_random_seeds
...
def run_inference(model, cfg, store, out_coords, ic_mems):
    ...
    if "cyclone_tracking" in cfg:
        ...
        heights, height_coords = load_heights(cfg.orography_path) if "orography_path" in cfg else (None, None)

        tracker = AsyncTempestExtremes if cfg.cyclone_tracking.asynchronous else TempestExtremes
        cyclone_tracking = tracker(detect_cmd=cfg.cyclone_tracking.detect_cmd,
    ...
    for ic, mems, seed in ic_mems:
        ...
        if ic != ic_prev:
            ...
            x0, coords0 = fetch_data(data_source,
        ...
        for _, (xx, coords) in tqdm(zip(range(cfg.n_steps+1), iterator), total=cfg.n_steps+1):
            write_to_store(store, xx, coords, out_coords)
            if cyclone_tracking:
                cyclone_tracking.record_state(xx, coords)

        if cyclone_tracking:
            cyclone_tracking(out_file_names=[f"tracks_{np.datetime_as_string(ic, unit='s')}_mem_{mem:04d}_seed_{seed}_bs_{cfg.batch_size}.csv" for mem in mems])
...
def configure_runs(cfg):
    ...
    seeds = get_set_of_random_seeds(n_ics=len(ics),
...
def set_reproduction_configs(cfg):
    ...
    for ii in range(len(ic_mems)):
        ...
        ic_mems[ii][1] = np.arange(batch_id*cfg.batch_size, min((batch_id+1)*cfg.batch_size, cfg.ensemble_size))
...
def generate_ensemble(cfg):
    ...
    store, out_coords = run_with_rank_ordered_execution(  # TODO: wrap only zarr store in that loop
        setup_output, cfg=cfg, model=model, ics=ics, add_arrays=DistributedManager().rank == 0)
